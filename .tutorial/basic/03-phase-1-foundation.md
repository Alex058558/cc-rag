...
> 完整術語定義見 [RAG 名詞表](07-rag-glossary.md)

- **JWT**：有時效的身份通行證。前端每次呼叫 API 都帶上它，過期了要重新換票。
- **RLS**：資料庫行級權限，不用每個 query 手刻 `where user_id = ...`。
- **Migration**：用 SQL 檔案描述 schema 變更，像 git commit 可追蹤可重現。
- **CORS**：瀏覽器安全機制，控制哪些網域能呼叫你的 API。
